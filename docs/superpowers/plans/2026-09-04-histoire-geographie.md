...
const TRANCHES={
  ...
  t5:{label:"1976-2002 · la fin de la guerre froide et l'Europe",court:"1976-2002",de:1976,a:2002,v:"--t5"}
};
...
const DATES=[
 ...
 {d:"1939-1945",an:1939,t:"t3",label:"Seconde Guerre mondiale",w:"Six ans de guerre mondiale, déclenchée par l'invasion de la Pologne le 1er septembre 1939."},
...
const FRISE_X0=20, FRISE_X1=320, FRISE_MIN=46;
/* les bornes de l'axe se deduisent des tranches : l'axe va de 1914 a 2002, pas
   de 1910 a 2010. Les coder en dur decrit un axe qui n'existe pas. */
const FRISE_DE=TRANCHES[TORDRE[0]].de, FRISE_A=TRANCHES[TORDRE[TORDRE.length-1]].a;
const FRISE_GEO=(()=>{
...
/* position exacte d'une annee sur l'axe, pour le repere affiche apres reponse.
   Interpoler sur la largeur du rectangle dessine (g.w-2) et non sur la largeur
   allouee : sinon le repere de la derniere annee d'une tranche tombe dans la
   gouttiere, juste a cote de la bande qu'on designe a l'eleve. */
function friseX(an){
  const id=TORDRE.find(k=>an>=TRANCHES[k].de&&an<=TRANCHES[k].a)||TORDRE[TORDRE.length-1];
  const t=TRANCHES[id], g=FRISE_GEO[id];
  return g.x+(an-t.de+.5)/(t.a-t.de+1)*(g.w-2);
}
/* opts : {bonne, choisie, an} tous optionnels */
function friseSVG(opts={}){
  const {bonne,choisie,an}=opts;
  const bandes=TORDRE.map(id=>{
    const t=TRANCHES[id], g=FRISE_GEO[id];
    let cls="tr";
    if(bonne&&id===bonne)cls+=" good";
    else if(choisie&&id===choisie)cls+=" bad";
    else if(bonne)cls+=" dim";
    /* une fois la reponse revelee les bandes n'ecoutent plus rien : leur retirer
       role et tabindex, sinon cinq boutons restent annonces et tabulables alors
       qu'ils sont inertes. */
    const inter=bonne?"":` role="button" tabindex="0"`;
    /* la cible transparente couvre la largeur pleine, gouttiere comprise : sans
       elle la zone tactile serait amputee des 2 unites de gouttiere. */
    return `<g class="${cls}" data-tr="${id}"${inter} aria-label="${esc(t.label)}">
      <rect class="cible" x="${g.x.toFixed(1)}" y="34" width="${g.w.toFixed(1)}" height="60"></rect>
  ...
  const repere=(bonne&&an!=null)
    ...
    : "";
  /* viewBox resserre exactement sur les bandes : les 40 unites de marge morte
     coutaient 12 % d'echelle, donc de largeur de cible, sur un telephone. */
  return `<svg viewBox="20 0 300 118" role="img" aria-label="Frise chronologique de ${FRISE_DE} \u00e0 ${FRISE_A}">
    <line class="axe" x1="${FRISE_X0}" y1="102" x2="${FRISE_X1}" y2="102"></line>
    ${bandes}${repere}
    <text x="${FRISE_X0}" y="115" class="borne">${FRISE_DE}</text>
    <text x="${FRISE_X1}" y="115" text-anchor="end" class="borne">${FRISE_A}</text>
  </svg>`;
}
```
...
.fig svg{width:100%;max-width:320px;height:auto;overflow:visible}
.fig .axe{stroke:var(--ink-soft);stroke-width:1}
.fig .tr .cible{fill:transparent;cursor:pointer}
.fig .tr rect:not(.cible){cursor:pointer;opacity:.82;transition:opacity .15s,transform .15s}
.fig .tr:hover rect:not(.cible),.fig .tr:focus rect:not(.cible){opacity:1}
.fig .tr:focus{outline:none}
.fig .tr:focus rect:not(.cible){stroke:var(--ink);stroke-width:2}
.fig .trlab{font-family:var(--sans);font-size:9.5px;font-weight:700;fill:#fff;pointer-events:none}
.fig .good rect:not(.cible){opacity:1;stroke:var(--ink);stroke-width:2}
.fig .bad rect:not(.cible){opacity:.35;stroke:var(--marge);stroke-width:2;stroke-dasharray:3 2}
.fig .dim rect:not(.cible){opacity:.3}
.fig .dim .trlab,.fig .bad .trlab{fill:var(--ink)}
...
.revealed .fig .tr rect,.revealed .fig .cible{cursor:default}
@media (prefers-reduced-motion:reduce){
  .fig .tr rect:not(.cible){transition:none}
}
```
